...
fig1.DockControls    = 'on';
fig1.WindowStyle    = 'docked';
cmap = jet(num_electrode); 

subplot(211)
for k=1:num_electrode
    hold on
    plot(time_ms/1000, HP_Signal_fix(:, k), 'Color', cmap(k, :));
...
% plot(sLocation, idx, '.');
for m=1:num_electrode
plot(sLocation(idx==m,:), avalanche_rot(idx==m, :), '.', 'Color', cmap(m, :));
 xlim([min(sLocation) max(sLocation)]) 
end
hold off
...
cspy(Stt, 'colormap', 'jet', 'MarkerSize', 10);
